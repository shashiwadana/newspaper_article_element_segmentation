...
def contWords(imNum):
    num = str(imNum)
    path = 'C:/Users/Shashi/Documents/FYP/FYP_Final/Outputs/' + num + '/columnwords/'
    os.mkdir(path)
    roilist = []
    imgs = [cv2.imread(fn, cv2.IMREAD_GRAYSCALE) for fn in glob('C:/Users/Shashi/Documents/FYP/FYP_Final/Outputs/' + num + '/columnLines/'+ '*.jpg')]
    for q in range(len(imgs)):
        image = cv2.imread('C:/Users/Shashi/Documents/FYP/FYP_Final/Outputs/' + num + '/columnLines/'+ str(q)+'.jpg', cv2.IMREAD_GRAYSCALE)
        ...
        if(len(heights) > 0):
            ...
            inBlock = False
            j = 0
            pix = math.floor(height * 0.6)
            whiteImage = np.ones((3, 5), dtype="uint8") * 255
